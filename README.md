...
# Run PageIndex
python run_pageindex_server.py --pdf_path path\to\your\document.pdf
...
Create a `.env` file in the root directory (see `.env.example`). Indexing and LLM calls use the [PageIndex cloud API](https://docs.pageindex.ai/endpoints) (your PageIndex credits):
...
Get your API key from the [Developer Dashboard](https://dash.pageindex.ai/api-keys).

### 3. Generate PageIndex structure for your PDF

```bash
python3 run_pageindex_server.py --pdf_path /path/to/your/document.pdf
```
...
```bash
python3 run_pageindex_server.py --md_path /path/to/your/document.md
```
...
For a simple, end-to-end _**agentic vectorless RAG**_ example using PageIndex API (with OpenAI Agents SDK), see [`examples/agentic_vectorless_rag_demo.py`](examples/agentic_vectorless_rag_demo.py).
...
#   i n g e s t i o n _ p a g e i n d e x i n g _ s u m  
 